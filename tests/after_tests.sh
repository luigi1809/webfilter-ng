...
[ "$(curl --max-time 5 -s -o /dev/null -w "%{http_code}" http://$google/ -H "Host: www.google.com")" == "200" ] && grep google.com /var/log/webfilter-ng | tail -n1 | grep DROP && exit 1
grep google.com /var/log/webfilter-ng | tail -n1 | grep DROP || exit 1


# test with forcing IP HTTPS
echo test with forcing IP HTTPS
[ "$(curl -k --max-time 5 -s -o /dev/null -w "%{http_code}" https://$google/ -H "Host: www.google.com")" == "200" ] && grep google.com /var/log/webfilter-ng | tail -n1 && curl -k --max-time 5 -s -o /dev/null -w "%{http_code}" https://$google/ -H "Host: www.google.com" && exit 1
grep google.com /var/log/webfilter-ng | tail -n1 | grep DROP || exit 1
