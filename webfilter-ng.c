...
bool webGuard(u_short proto, char **host, char **uri, char **saddr, char **daddr, u_short sport, u_short dport){
    char * cmd;
...
bool check_packet_against_hostname(const unsigned char *packet)
{
	...
	* daddr='\0';
	...
	//Set address of TCP data in memory to data
	data = (char *)((unsigned char *)tcp_header + (tcp_header->doff * 4));
	//tail = skb_tail_pointer(skb);
	//// Calculate packet data length
	//tcpdatalen = (uintptr_t)tail - (uintptr_t)data;
	//printf("\n%s\n",data);
	//for(u_int edf=0; edf<tcpdatalen; edf++){
	//	printf("%02x", (unsigned int) data[edf]);
	//}
	//printf("\n");
			...
			if (handshake_protocol == 0x01) {
				...
				while (extension_offset < extensions_len)
				{
					...
					if (extension_id == 0) {
						...
						uri[0]='\0';
						printf("SNI: %s\n", host);
						return webGuard(HTTPS_PROTO,&host,&uri,&saddr,&daddr,sport,dport);
						free(saddr);
						free(daddr);
						free(host);
					}
			...
			for(i; i<tcpdatalen ; i++){	
				if((data[i] == 'H' || data[i] == 'h') && (data[i+1] == 'O' || data[i+1] == 'o') && (data[i+2] == 's'|| data[i+2] == 's') && (data[i+3] == 'T' || data[i+3] == 't') && data[i+4] == ':'){
					...
					*host='\0';
					...
					host[i-j]='\0';
					printf("URL http://%s%s\n",host,uri);
					return webGuard(HTTP_PROTO,&host,&uri,&saddr,&daddr,sport,dport);					
				}
			}
		}
	}
